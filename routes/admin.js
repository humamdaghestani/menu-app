...
router.post('/items', requireAuth, async (req, res) => {
  ...
  try {
    await db.query(
      `INSERT INTO menu_items (tenant_id, category_id, name, price, description, image_url)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [req.user.tenantId, category_id, name, price, description, image_url]
    );
    res.redirect('/admin/dashboard');
  } catch (err) {
    console.error(err);
...
router.post('/items/:id/edit', requireAuth, async (req, res) => {
  ...
  try {
    await db.query(
      `UPDATE menu_items SET name=$1, price=$2, description=$3, image_url=$4, category_id=$5
       WHERE id=$6 AND tenant_id=$7`,
      [name, price, description, image_url, category_id, req.params.id, req.user.tenantId]
    );
    res.redirect('/admin/dashboard');
  } catch (err) {
    console.error(err);
