...
router.get('/print-agent-installer', requireAuth, requirePOS, (req, res) => {
  ...
  const agentLines = [
    ...
    "    cmd(0x1B,0x61,0x00);",
    "    txt(sep('-')+'\n');",
    "    txt((data.order_type||'Dine-in')+'  Bill #'+(data.bill_id||'')+'\n');",
    "    txt(rpad(data.table||'', pw-22)+(data.date||'')+'\n');",
    "    if (data.note) { txt(sep('-')+'\n'); txt('Note: '+data.note+'\n'); }",
    "    txt(sep('=')+'\n');",
    "    cmd(0x1B,0x45,0x01);",
    "    txt(row4('Item','Qty','Price','Total'));",
    "    cmd(0x1B,0x45,0x00);",
    "    txt(sep('-')+'\n');",
    ...
    "      if (nm.length>NW) {",
    "        txt(row4(nm.slice(0,NW),'x'+it.qty,upr,tot));",
    "        var r=nm.slice(NW); while(r.length){txt(rpad(' '+r.slice(0,NW-1),NW)+'\n');r=r.slice(NW-1);}",
    "      } else { txt(row4(nm,'x'+it.qty,upr,tot)); }",
    "      if (it.note) txt(' >> '+it.note+'\n');",
    "    }",
    "    txt(sep('=')+'\n');",
    "    if (data.subtotal) txt(ln2(lbls.subtotal||'Sub Total', fmtAmt(data.subtotal))+'\n');",
    "    if (data.svcfee)   txt(ln2((lbls.svcfee||'Service')+' +', fmtAmt(data.svcfee))+'\n');",
    "    if (data.discount) txt(ln2((lbls.discount||'Discount')+' -', fmtAmt(data.discount))+'\n');",
    "    cmd(0x1B,0x45,0x01);",
    "    txt(ln2(lbls.total||'Net Total', fmtAmt(data.total))+'\n');",
    "    cmd(0x1B,0x45,0x00);",
    "    if (data.iqd_total) txt(ln2('IQD', data.iqd_total)+'\n');",
    "    txt(sep('-')+'\n');",
    "    cmd(0x1B,0x61,0x01);",
