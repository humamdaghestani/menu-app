...
router.get('/print-agent-installer', requireAuth, requirePOS, (req, res) => {
  ...
  const agentLines = [
    ...
    "  function san(s) { return String(s||'').split(' - ').map(function(p){ return p.replace(/[^\\x20-\\x7E]/g,'').trim(); }).filter(function(p){ return p.length>0; }).join(' / '); }",
    ...
    "    txtnl(san(data.shop||''));",
    "    cmd(0x1B,0x21,0x00);",
    "    if (data.header) txtnl(san(data.header));",
    "    cmd(0x1B,0x61,0x00);",
    "    txtnl(sep('-'));",
    "    txtnl(san(data.order_type||'Dine-in')+'  Bill #'+(data.bill_id||''));",
    "    txtnl(rpad(san(data.table||''), pw-22)+(data.date||''));",
    "    if (data.note) { txtnl(sep('-')); txtnl('Note: '+san(data.note)); }",
    "    txtnl(sep('='));",
    "    cmd(0x1B,0x45,0x01);",
    "    txtnl(row4('Item','Qty','Price','Total'));",
    "    cmd(0x1B,0x45,0x00);",
    "    txtnl(sep('-'));",
    ...
    "      if (it.note) txtnl(' >> '+san(it.note));",
    "    }",
    "    txtnl(sep('='));",
    "    if (data.subtotal) txtnl(ln2(san(lbls.subtotal||'Sub Total'), fmtAmt(data.subtotal)));",
    "    if (data.svcfee)   txtnl(ln2(san(lbls.svcfee||'Service')+' +', fmtAmt(data.svcfee)));",
    "    if (data.discount) txtnl(ln2(san(lbls.discount||'Discount')+' -', fmtAmt(data.discount)));",
    "    cmd(0x1B,0x45,0x01);",
    "    txtnl(ln2(san(lbls.total||'Net Total'), fmtAmt(data.total)));",
    "    cmd(0x1B,0x45,0x00);",
    "    if (data.iqd_total) txtnl(ln2('IQD', String(data.iqd_total)));",
    "    txtnl(sep('-'));",
    "    cmd(0x1B,0x61,0x01);",
    "    if (lbls.cashier) txtnl(san(lbls.served_by||'Served by')+': '+san(lbls.cashier));",
    "    txtnl(san(data.footer||lbls.thanks||'Thank you!'));",
    "  } else if (data.type === 'order') {",
    "    cmd(0x1B,0x61,0x01); cmd(0x1B,0x21,0x30);",
    "    txtnl(san(data.table||'Takeaway'));",
    ...
    "    if (data.order_type) txtnl(san(data.order_type));",
    "    if (data.note) txtnl('Note: '+san(data.note));",
    "    txtnl(sep('-'));",
    "    var items=data.items||[];",
    "    for (var ii=0;ii<items.length;ii++) {",
    "      var it=items[ii];",
    "      cmd(0x1B,0x45,0x01);",
    "      txtnl('x'+it.qty+'  '+san(it.name));",
    "      cmd(0x1B,0x45,0x00);",
    "      if (it.note) txtnl('   >> '+san(it.note));",
    "    }",
    "    txtnl(sep('='));",
