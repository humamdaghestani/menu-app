...
// ── Print Agent token-based JS download (used by install bat) ────────────────
router.get('/print-agent-dl/:token', async (req, res) => {
  try {
    const tRes = await db.query('SELECT * FROM tenants WHERE pos_agent_token=$1', [req.params.token]);
    if (!tRes.rows[0]) return res.status(403).send('Invalid token');
    const relayBase = req.protocol + '://' + req.get('host');
    const tok = req.params.token;
    // reuse same agentLines as print-agent-js
    const agentLines = [
      "var http  = require('http');",
      "var https = require('https');",
      "var url   = require('url');",
      ...
      "  res.setHeader('Access-Control-Allow-Headers','Content-Type');",
      "  if (req.method==='OPTIONS') { res.writeHead(204); res.end(); return; }",
      "  if (req.method==='GET' && req.url==='/ping') { res.writeHead(200,{'Content-Type':'application/json'}); res.end(JSON.stringify({ok:true,version:'3.0'})); return; }",
      "  if (req.method==='POST' && req.url==='/print') {",
      "    var body='';",
      "    req.on('data',function(d){ body+=d.toString(); });",
      "    req.on('end',function() {",
      ...
      "  var parsed = url.parse(RELAY_URL+'/pos/agent/poll?token='+AGENT_TOKEN);",
      "  var mod = parsed.protocol==='https:'?https:http;",
      "  mod.get({host:parsed.hostname,port:parsed.port,path:parsed.path,headers:{'User-Agent':'POSAgent/3.0'}},function(res){",
      "    var body=''; res.on('data',function(d){body+=d;}); res.on('end',function(){",
      "      try {",
      "        var r=JSON.parse(body);",
      "        (r.jobs||[]).forEach(function(job){",
...
// ── Print Agent install BAT — self-contained, no exe needed ──────────────────
router.get('/print-agent-install-bat', requireAuth, requirePOS, async (req, res) => {
  ...
  const bat = [
    ...
    '',
    ':: Get full path to node.exe (embedded in VBS so wscript finds it regardless of PATH)',
    'for /f "delims=" %%i in (\'where node\') do (set "NODEEXE=%%i" & goto :nodeFound)',
    ':nodeFound',
    'echo  [OK] Node path: %NODEEXE%',
    '',
    ':: Set paths',
    ...
    '',
    ':: Write VBS launcher using full node path (window style 0 = hidden)',
    '(',
    '  echo Dim sh',
    '  echo Set sh = CreateObject("WScript.Shell"^)',
    '  echo sh.Run Chr(34^) ^& "%NODEEXE%" ^& Chr(34^) ^& " " ^& Chr(34^) ^& "%AGENTJS%" ^& Chr(34^), 0, False',
    ') > "%AGENTVBS%"',
