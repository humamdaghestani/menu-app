...
router.get('/print-agent-installer', requireAuth, requirePOS, (req, res) => {
  ...
  const agentLines = [
    ...
    "0x00E0,0x0644,0x00E2,0x0645,0x0646,0x0647,0x0648,0x00E7,0x00E8,0x0649,0x064A,0x00EB,0x00EC,0x064B,0x064C,0x064D,",
    "0x064E,0x064F,0x0650,0x0651,0x0652,0x00F5,0x00F6,0x00F7,0x00F8,0x00F9,0x00FA,0x00FB,0x00FC,0x200E,0x200F,0x06D2",
    "].forEach(function(u,i){U2CP[u]=i+0x80;});",
    "U2CP[0x06CC]=0xEA;",
    "function buildEscPos(data) {",
    "  var buf = [];",
    "  function txt(s){var str=String(s||'');for(var i=0;i<str.length;i++){var c=str.charCodeAt(i);if(c<0x80)buf.push(c);else if(U2CP[c]!==undefined)buf.push(U2CP[c]);else buf.push(0x3F);}}",
