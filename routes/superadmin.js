...
router.post('/tenants/:id/delete', requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    // Nullify cross-references between sibling tables (both cascade from tenant)
    ...
    // Explicitly delete tables that may lack CASCADE in older DB instances
    await db.query(`DELETE FROM valet_requests WHERE tenant_id=$1`, [tid]);
...
router.post('/tenants/:id/features', requireSuperAdmin, async (req, res) => {
  try {
    const features = ['feat_feedback','feat_orders','feat_import','feat_custom_css','feat_multilang','feat_valet','feat_splash_custom','feat_cart','feat_pos','feat_captain','feat_inventory','feat_accounting','feat_hr','feat_reservations','feat_loyalty','feat_delivery','feat_assets','feat_reports','feat_modifiers','feat_happy_hour','feat_kitchen_routing','feat_no_sale','feat_prep_time'];
    ...
    const result = await db.query(
      `UPDATE tenants SET
        ...
        feat_modifiers=$19, feat_happy_hour=$20, feat_kitchen_routing=$21, feat_no_sale=$22, feat_prep_time=$23
       WHERE id=$24`,
